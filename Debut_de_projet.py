...
def update_figure_timeline(input_value,drop_input):
    ...
    for ville in drop_input:
        ordonnee.append(df[df["LIBZE2020"]==ville].iloc[:,5+input_value[0]:5+input_value[1]].iloc[0])
    #print(ordonnee)
    ...
    return [
        fig,
    f'Evolution entre ({pd.to_datetime(df.columns[5+input_value[0]])}) à ({pd.to_datetime(df.columns[5+input_value[1]])})',
        f'Dans la ville de ({drop_input})'
    ]
